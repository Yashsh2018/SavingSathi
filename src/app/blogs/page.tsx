...
import { Loader2, ArrowLeft, Home, CalendarDays, User, Clock } from 'lucide-react';
...
function BlogPageContent() {
    ...
    const fetchBlogData = async () => {
        try {
            ...
            if (data.status && data.data) {
                ...
                // If blogId exists, find and set the specific blog
                if (blogId) {
                    const foundBlog = data.data.find(b => b.id.toString() === blogId);
                    if (foundBlog) {
                        setBlog(foundBlog);
                    } else {
    ...
    if (blog) {
        return (
            <div className="min-h-screen bg-black">
                {/* Blog content */}
                <article className="container mx-auto px-4 pb-8 max-w-4xl pt-20">
                    {/* Header */}
                    <header className="my-8">
                        <div className="flex items-center text-sm text-gray-400 mb-4">
                            <span className="bg-gradient-to-r from-rose-600 to-cyan-700 text-white font-semibold px-3 py-1 rounded-full">
                                Published
                            </span>
                        ...
                        <div className="flex items-center">
                            {blog.profile_image ?
                                <img
                                    ...
                                    className="w-10 h-10 rounded-full items-center mr-3"
                                    onError={(e) => {
                                        const target = e.target as HTMLImageElement;
                                        target.style.display = 'none';
                                        target.parentElement!.innerHTML = `
                                            <div class="w-10 h-10 bg-gradient-to-br from-rose-600 to-cyan-700 rounded-full flex items-center justify-center text-white font-bold mr-3">
                                ...
                                />
                                :
                                <div className="w-10 h-10 bg-gradient-to-br from-rose-600 to-cyan-700 rounded-full flex items-center justify-center text-white font-bold mr-3">
                                    {blog.author_name.charAt(0)}
                                </div>
                            }
                            <div>
                        ...
                        <div className="mt-12 pt-8 border-t border-gray-700">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {blogs
                                    ...
                                    .slice(0, 3)
                                    .map((relatedBlog) => (
                                        <Link key={relatedBlog.id} href={`/blogs?id=${relatedBlog.id}`}>
                                            <div className="bg-[#242424] p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow cursor-pointer border border-gray-700 h-full">
                    ...
                    <div className="mt-12 pt-8 border-t border-gray-700 flex justify-between">
                        <Link
                            href="/blogs"
                            className="flex items-center text-gray-300 hover:text-cyan-400 transition-colors font-semibold"
                        >
